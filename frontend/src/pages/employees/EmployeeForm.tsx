...
// import { useNavigate, useParams } from 'react-router-dom';
// import {
//   ArrowLeft, Save, Loader2, User, Briefcase, Landmark,
//   UserPlus, Eye, EyeOff, MapPin, Building2,
// } from 'lucide-react';
// import Sidebar from '../../components/Sidebar';
// import Topbar from '../../components/Topbar';
// import { employeesApi, type EmployeeCreatePayload } from '../../api/employees';
...
//   // Master data
//   const [allStructures, setAllStructures] = useState<CompanyStructure[]>([]);
//   const [hasExistingAccount, setHasExistingAccount] = useState(false);
//   const [positions, setPositions] = useState<JobPosition[]>([]);
//   const [managers, setManagers] = useState<ManagerOption[]>([]);
//   const [roles, setRoles] = useState<Role[]>([]);

//   // Derived lists from allStructures
//   const departments = allStructures.filter((s) =>
//     ['DEPARTMENT', 'TEAM'].includes(s.type) && s.is_active
//   );
//   const locations = allStructures.filter((s) =>
//     ['LOCATION', 'HQ', 'COMPANY'].includes(s.type) && s.is_active
//   );
//   const costCenters = allStructures.filter((s) =>
//     s.type === 'COST_CENTER' && s.is_active
//   );
...
//         const structures = structData.results || structData;
//         setAllStructures(Array.isArray(structures) ? structures : []);
//         setPositions(posData.results || []);
//         setManagers(mgrData);
//         setRoles(roleData.results || []);
...
//             position: emp.position?.id || null,
//             reporting_manager: emp.reporting_manager?.id || null,
//             // Keep backward compat — use structure_location as department fallback
//             structure_location: (emp as any).department?.id || emp.structure_location?.id || null,
//             date_of_joining: emp.date_of_joining,
//             date_of_exit: emp.date_of_exit,
//             bank_ifsc_code: emp.bank_ifsc_code || '',
//           });
...
//     setSelectedDepartment(empData.department_detail?.id || emp.structure_location?.id || '');
//     setSelectedLocation(empData.location_detail?.id || '');
//     setSelectedCostCenter(empData.cost_center_detail?.id || '');
...
//         position: form.position || null,
//         reporting_manager: form.reporting_manager || null,
//         // Send structure_location as department for backward compat
//         structure_location: selectedDepartment || form.structure_location || null,
//         date_of_exit: form.date_of_exit || null,
//         // New hierarchy fields
//         department: selectedDepartment || null,
//         location: selectedLocation || null,
//         cost_center: selectedCostCenter || null,
//       };
...
//   // Get hierarchy path for selected department
//   const getHierarchyPath = (): string => {
//     if (!selectedDepartment) return '';
//     const dept = allStructures.find((s) => s.id === selectedDepartment);
//     return dept?.path || '';
//   };
...
//             <SectionCard icon={Building2} title="Organization Hierarchy">
//               <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
...
//                       // Also update old field for backward compat
//                       update('structure_location', e.target.value || null);
//                     }}
//                     className={inputCls()}
//                   >
...
//                         {d.path ? `${d.path.split('/').slice(-2, -1)[0]} → ` : ''}{d.name}
//                         {d.type === 'TEAM' ? ' (Team)' : ''}
//                       </option>
//                     ))}
//                   </select>
...
//                         {l.name}
//                         {l.type === 'HQ' ? ' (HQ)' : l.type === 'COMPANY' ? ' (Company)' : ''}
//                       </option>
//                     ))}
//                   </select>
//                 </Field>
...
//                         {cc.cost_center_code ? `${cc.cost_center_code} — ` : ''}{cc.name}
//                       </option>
//                     ))}
//                   </select>
//                 </Field>
...
//                   <p className="text-xs font-semibold text-blue-700">Hierarchy Path</p>
//                   <p className="mt-1 font-mono text-sm text-blue-900">{getHierarchyPath()}</p>
//                 </div>
//               )}
//             </SectionCard>
...
//                 {isEditMode && !hasExistingAccount && (
//                   <p className="mt-2 text-xs text-gray-500">
//                     This employee doesn't have a login account yet. Check above to create one.
//                   </p>
//                 )}
...
import {
  ArrowLeft, Save, Loader2, User, Briefcase, Landmark,
  UserPlus, Eye, EyeOff, Building2, Home,
} from 'lucide-react';
import Sidebar from '../../components/Sidebar';
import Topbar from '../../components/Topbar';
import { employeesApi, type EmployeeCreatePayload } from '../../api/employees';
...
export default function EmployeeForm() {
      ...
      try {
        ...
        if (isEditMode && id) {
          ...
          setForm({
            ...
            date_of_exit: emp.date_of_exit,
            // === NEW ===
            work_mode: (emp as any).work_mode || 'REGULAR',
            can_self_attend: (emp as any).can_self_attend ?? false,
            // === END NEW ===
            bank_ifsc_code: emp.bank_ifsc_code || '',
          });
  ...
  // === NEW: Work-mode change handler with auto-sync ===
  const handleWorkModeChange = (newMode: 'REGULAR' | 'WFH' | 'HYBRID') => {
    setForm((f) => ({
      ...f,
      work_mode: newMode,
      // Auto-enable self-attendance for WFH/HYBRID, disable for REGULAR
      can_self_attend: newMode === 'WFH' || newMode === 'HYBRID',
    }));
  };
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto p-6">
          ...
          <div className="mb-6">
            ...
            <p className="mt-1 text-sm text-gray-600">
              {isEditMode ? 'Update employee details' : 'Fill in the details. Employee ID will be auto-generated.'}
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="mx-auto max-w-4xl space-y-6">
            ...
            <SectionCard icon={Briefcase} title="Employment Details">
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                ...
                <Field label="Status">
                  <select value={form.status || 'PROBATION'}
                    onChange={(e) => update('status', e.target.value)}
                    className={inputCls()}>
                    <option value="PROBATION">Probation</option>
                    <option value="ACTIVE">Active</option>
                ...
                <Field label="Reporting Manager">
                  <select value={form.reporting_manager || ''}
                    onChange={(e) => update('reporting_manager', e.target.value || null)}
                    className={inputCls()}>
                    <option value="">— None —</option>
                    {managers.map((m) => (
                ...
                {isEditMode && (
                  <Field label="Date of Exit">
                    <input type="date" value={form.date_of_exit || ''}
                      onChange={(e) => update('date_of_exit', e.target.value || null)}
                      className={inputCls()} />
                  </Field>
                )}
            ...
            {/* === NEW: Work Arrangement === */}
                      {/* === NEW: Work Arrangement === */}
            <SectionCard icon={Home} title="Work Arrangement">
              <div className="grid grid-cols-1 items-start gap-6 md:grid-cols-2">
                
                {/* Left Column: Work Mode */}
                <div>
                  <label className="mb-1 block text-sm font-medium text-gray-700">Work Mode *</label>
                  <select
                    value={form.work_mode || 'REGULAR'}
                    onChange={(e) => handleWorkModeChange(e.target.value as 'REGULAR' | 'WFH' | 'HYBRID')}
                    className={inputCls()}
                  >
                  ...
                  <p className="mt-1.5 text-xs text-gray-500">
                    {form.work_mode === 'REGULAR' && 'Must punch on biometric device daily.'}
                    {form.work_mode === 'WFH' && 'Can self-report attendance from their calendar.'}
                    {form.work_mode === 'HYBRID' && 'Can use biometric device AND self-report.'}
                  </p>
                </div>

                {/* Right Column: Self Attendance Checkbox */}
                <div>
                  <label className="mb-1 block text-sm font-medium text-gray-700">Self-Attendance Access</label>
                  <div className="flex items-center gap-3 rounded-lg border border-gray-200 bg-gray-50 p-3">
                    <input
                      id="can_self_attend"
                      type="checkbox"
                      checked={form.can_self_attend || false}
                      onChange={(e) => update('can_self_attend', e.target.checked)}
                      className="h-4 w-4 rounded border-gray-300 text-primary-600 focus:ring-primary-500"
                    />
                    <label htmlFor="can_self_attend" className="flex-1 cursor-pointer text-sm">
                      <span className="block font-semibold text-gray-900">Allow self-attendance</span>
                      <span className="block mt-0.5 text-xs text-gray-500">
                        Can add WFH / Site Visit entries from calendar
                      </span>
                    </label>
                  </div>
                </div>
              ...
              {/* Info banner - Now safely below the grid */}
              {form.work_mode !== 'REGULAR' && form.can_self_attend && (
                <div className="mt-5 rounded-lg border border-indigo-200 bg-indigo-50 p-3">
                  <p className="text-xs text-indigo-800">
                    ✅ This employee will see a <strong>+ Add</strong> button on absent days in their attendance calendar to self-report WFH or Site Visit.
                  </p>
                </div>
              )}
              {form.work_mode === 'REGULAR' && !form.can_self_attend && (
                <div className="mt-5 flex gap-2 rounded-lg border border-amber-200 bg-amber-50 p-3">
                  ...
                  <p className="text-xs text-amber-800">
                    This employee must punch on the biometric device daily. Self-attendance is disabled.
                  </p>
                </div>
              )}
            </SectionCard>
            ...
            <SectionCard icon={Building2} title="Organization Hierarchy">
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                ...
                <Field label="Cost Center">
                  <select value={selectedCostCenter}
                    onChange={(e) => setSelectedCostCenter(e.target.value)}
                    className={inputCls()}>
                    <option value="">— None —</option>
                    {costCenters.map((cc) => (
            ...
            <SectionCard icon={Landmark} title="Bank & Statutory (Optional)">
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                <Field label="Bank Account Number">
                  <input type="text" value={form.bank_account_encrypted || ''}
                    onChange={(e) => update('bank_account_encrypted', e.target.value)}
                    className={inputCls()} />
                </Field>
                <Field label="IFSC Code">
                  <input type="text" value={form.bank_ifsc_code || ''}
                    onChange={(e) => update('bank_ifsc_code', e.target.value.toUpperCase())}
                    className={inputCls()} />
                </Field>
                <Field label="PAN Number">
                  <input type="text" value={form.pan_number_encrypted || ''}
                    onChange={(e) => update('pan_number_encrypted', e.target.value.toUpperCase())}
                    className={inputCls()} />
                </Field>
                <Field label="Aadhaar Number">
                  <input type="text" value={form.aadhaar_number_encrypted || ''}
                    onChange={(e) => update('aadhaar_number_encrypted', e.target.value)}
                    className={inputCls()} />
                </Field>
                <Field label="UAN Number">
                  <input type="text" value={form.uan_number_encrypted || ''}
                    onChange={(e) => update('uan_number_encrypted', e.target.value)}
                    className={inputCls()} />
                </Field>
              </div>
            ...
            {(!isEditMode || (isEditMode && !hasExistingAccount)) && (
              <SectionCard icon={UserPlus} title="Login Account (Optional)">
                <label className="flex items-center gap-2 text-sm">
                  <input type="checkbox" checked={form.create_user_account || false}
                    onChange={(e) => update('create_user_account', e.target.checked)}
                    className="h-4 w-4 rounded" />
                  Create login account for this employee
                </label>
