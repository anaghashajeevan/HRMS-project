...
// type LoadingAction = 'save' | 'test_essl' | 'test_email' | 'generate_monthly' | 'send_monthly' | null;
// type SettingsTab = 'essl' | 'email' | 'rules' | 'automation' | 'monthly';
...
//   const [action, setAction] = useState<LoadingAction>(null);
//   const [toast, setToast] = useState<ToastState | null>(null);
//   const [activeTab, setActiveTab] = useState<SettingsTab>('essl');
...
//     setEditingDevice(null);
//     const available = getAvailableRoles();
//     setDeviceForm({ name: '', serial_number: '', role: (available[0]?.value as any) || 'BOTH', is_active: true });
//     setDeviceModalOpen(true);
//   };

//   const handleOpenEditDevice = (device: EsslDevice) => {
//     setEditingDevice(device);
//     setDeviceForm({ name: device.name, serial_number: device.serial_number, role: device.role, is_active: device.is_active });
//     setDeviceModalOpen(true);
//   };
...
//     { id: 'essl', label: 'eSSL & Devices', icon: ServerCog },
//     { id: 'email', label: 'Email (SMTP)', icon: Mail },
//     { id: 'rules', label: 'Rules & Period', icon: ShieldCheck },
...
//         <h1 className="mt-2 text-3xl font-semibold text-white">Automation Settings</h1>
//         <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-400">
//           Manage eSSL, SMTP, attendance rules, and schedule values. Use the tabs below to navigate.
//         </p>
//       </header>
...
//       <form className="relative pb-24" onSubmit={(e) => e.preventDefault()}>
//         <div className="grid gap-5 xl:grid-cols-2">
          
//           {/* ================= TAB 1: ESSL & DEVICES ================= */}
//           {activeTab === 'essl' && (
//             <SettingsSection title="eSSL API & Devices">
//               <SectionLabel icon={ServerCog} text="Global Connection Credentials" />
//               <FormField label="eSSL API URL" value={settings.essl_api_url} onChange={(e) => updateField('essl_api_url', e.target.value)} />
//               <FormField label="API Username" value={settings.api_username} onChange={(e) => updateField('api_username', e.target.value)} />
//               <SecretField label="API Password" name="api_password" status={settings.secret_statuses.api_password} value={apiPassword} onChange={(e: ChangeEvent<HTMLInputElement>) => setApiPassword(e.target.value)} />
...
//                         {deviceData.mode === 'EMPTY' && 'Fallback (.env / Global)'}
//                         {deviceData.mode === 'SINGLE' && '1 Active Device'}
//                         {deviceData.mode === 'MULTI' && `${activeDevices.length} Active Devices (${bothCount} Both, ${inCount} In, ${outCount} Out)`}
//                       </strong>
//                     </p>
//                   </div>
//                   {canAddDevice && (
//                     <Button type="button" onClick={handleOpenAddDevice}>
//                       <Plus className="h-4 w-4" aria-hidden="true" /> Add Device
//                     </Button>
//                   )}
//                 </div>
...
//                 {activeDevices.length > 0 && (
//                   <div className="mb-3 rounded-lg border border-blue-400/30 bg-blue-500/10 p-2.5 text-xs text-blue-200">
//                     ℹ️ Multi-device mode: Employees can punch on any device, and the system merges all punches per day (earliest = Punch In, latest = Punch Out).
//                   </div>
//                 )}
...
//                 ) : deviceData.devices.length === 0 ? (
//                   <div className="rounded-lg border border-white/10 bg-[#070A12]/80 p-4 text-center text-xs text-slate-400">
//                     No devices added yet. Click <strong>Add Device</strong> to configure.
//                   </div>
//                 ) : (
//                   <div className="space-y-2">
//                     {deviceData.devices.map((device) => (
...
//                           <div className="flex items-center gap-2">
//                             <span className="text-sm font-bold text-white">{device.name}</span>
//                             <span className={`rounded-full px-2 py-0.5 text-[10px] font-extrabold uppercase ${device.role === 'BOTH' ? 'bg-blue-500/20 text-blue-300 border border-blue-500/30' : device.role === 'PUNCH_IN' ? 'bg-green-500/20 text-green-300 border border-green-500/30' : 'bg-orange-500/20 text-orange-300 border border-orange-500/30'}`}>
//                               {device.role_display}
//                             </span>
//                             {!device.is_active && <span className="rounded-full bg-slate-800 px-2 py-0.5 text-[10px] text-slate-400">Inactive</span>}
//                           </div>
//                           <div className="mt-1 text-xs text-slate-400">
//                             Serial: <strong className="text-slate-200">{device.serial_number}</strong>
//                             {device.last_test_message && (
//                               <span className="ml-3 inline-flex items-center gap-1">
//                                 {device.last_test_ok ? <CheckCircle2 className="h-3 w-3 text-green-400" /> : <XCircle className="h-3 w-3 text-red-400" />}
//                                 <span className={device.last_test_ok ? 'text-green-300' : 'text-red-300'}>{device.last_test_message}</span>
//                               </span>
//                             )}
//                           </div>
//                         </div>
//                         <div className="flex items-center gap-2">
//                           <Button type="button" loading={testingDeviceId === device.id} onClick={() => handleTestDevice(device)} className="min-h-9 px-3 text-xs">
//                             <Wifi className="h-3.5 w-3.5" aria-hidden="true" /> Test
//                           </Button>
//                           <button type="button" onClick={() => handleOpenEditDevice(device)} className="rounded-lg border border-white/10 bg-white/[0.05] p-2 text-slate-300 hover:bg-white/10"><Edit3 className="h-3.5 w-3.5" /></button>
//                           <button type="button" onClick={() => handleRemoveDevice(device)} className="rounded-lg border border-red-400/30 bg-red-500/10 p-2 text-red-300 hover:bg-red-500/20"><Trash2 className="h-3.5 w-3.5" /></button>
//                         </div>
//                       </div>
//                     ))}
//                   </div>
...
//                 <SectionLabel icon={CalendarRange} text="When calendars start showing attendance" />
//                 <p className="mb-3 text-xs leading-5 text-slate-400">
//                   Months before this date show as "No attendance data". Leave on <strong>Auto</strong> to use the first month with punch records.
//                 </p>
//                 <label className="grid gap-2">
//                   <span className="text-sm font-medium text-slate-300">Start mode</span>
//                   <select value={settings.attendance_start_mode || 'AUTO'} onChange={(e) => updateField('attendance_start_mode', e.target.value as any)} className="min-h-11 w-full rounded-xl border border-white/10 bg-[#070A12]/80 px-3 text-sm text-slate-100 outline-none focus:border-blue-400/60">
//                     <option value="AUTO">Auto (from first punch / attendance data)</option>
//                     <option value="MANUAL">Manual (fixed month)</option>
//                   </select>
//                 </label>
//                            {(settings.attendance_start_mode || 'AUTO') === 'MANUAL' && (
//               <div className="grid gap-2">
//                 <span className="text-sm font-medium text-slate-300">Attendance starts from</span>
                
//                 {(() => {
//                   // Extract selected year and month (default to current year and month if empty)
//                   const now = new Date();
//                   const defaultVal = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}`;
//                   const currentVal = settings.attendance_start_month || defaultVal;
//                   const [selectedYear, selectedMonth] = currentVal.split('-');
...
//                   // Generate years list (e.g., 2020 to 2035)
...
//                       {/* Month Dropdown */}
//                       <div>
//                         <label className="mb-1 block text-xs text-slate-400">Select Month</label>
//                         <select
//                           value={selectedMonth || '01'}
//                           onChange={(e) => {
//                             const newMonth = e.target.value;
//                             updateField('attendance_start_month', `${selectedYear || now.getFullYear()}-${newMonth}`);
//                           }}
//                           className="min-h-11 w-full rounded-xl border border-white/10 bg-[#070A12]/80 px-3 text-sm text-slate-100 outline-none transition focus:border-blue-400/60"
//                         >
//                           {monthsList.map((m) => (
...
//                       {/* Year Dropdown */}
//                       <div>
//                         <label className="mb-1 block text-xs text-slate-400">Select Year</label>
//                         <select
//                           value={selectedYear || String(now.getFullYear())}
//                           onChange={(e) => {
//                             const newYear = e.target.value;
//                             updateField('attendance_start_month', `${newYear}-${selectedMonth || '01'}`);
//                           }}
//                           className="min-h-11 w-full rounded-xl border border-white/10 bg-[#070A12]/80 px-3 text-sm text-slate-100 outline-none transition focus:border-blue-400/60"
//                         >
//                           {yearsList.map((y) => (
...
//             )}
//                 <div className="mt-3 rounded-lg border border-white/10 bg-slate-950/45 px-3 py-2 text-xs text-slate-300">
//                   Resolved start: <strong className="text-white">{settings.resolved_attendance_start ? (() => { const [y, m] = settings.resolved_attendance_start.split('-'); return new Date(Number(y), Number(m) - 1, 1).toLocaleString('en-US', { month: 'long', year: 'numeric' }); })() : 'No data yet'}</strong>
//                 </div>
//               </SettingsSection>
//             </div>
//           )}
...
//               <label className="grid gap-2">
//                 <span className="text-sm font-medium text-slate-300">Automation Run Mode</span>
//                 <select value={settings.automation_run_mode} onChange={(e) => updateField('automation_run_mode', e.target.value as AutomationRunMode)} className="min-h-11 w-full rounded-xl border border-white/10 bg-[#070A12]/80 px-3 text-sm text-slate-100 outline-none focus:border-blue-400/60">
//                   <option value="DAILY">Daily</option>
//                   <option value="MONTHLY">Monthly</option>
//                   <option value="BOTH">Both</option>
//                 </select>
...
//                   <label className="grid gap-2">
//                     <span className="text-sm font-medium text-slate-300">Report Month</span>
//                     <input type="month" value={monthlyMonth} onChange={(e) => setMonthlyMonth(e.target.value)} className="min-h-11 w-full rounded-xl border border-white/10 bg-[#070A12]/80 px-3 text-sm text-slate-100 outline-none sm:max-w-xs" />
//                   </label>
//                   <div className="flex flex-wrap items-center justify-between gap-3">
//                     <div className="text-sm font-semibold text-slate-300">
//                       <span>Status: <strong className="text-white">{monthlyStatus}</strong></span>
...
//                     <div className="flex flex-wrap gap-3">
//                       <Button loading={action === 'generate_monthly'} onClick={generateMonthly} type="button">
//                         <FileSpreadsheet className="h-4 w-4" aria-hidden="true" /> Generate Excel
//                       </Button>
//                       <Button variant="primary" loading={action === 'send_monthly'} onClick={sendMonthly} type="button">
//                         <Send className="h-4 w-4" aria-hidden="true" /> Send Email
//                       </Button>
//                       {monthlyDownloadUrl && (
//                         <button onClick={downloadMonthly} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-white/10 bg-white/[0.055] px-4 text-sm font-bold text-slate-100 transition hover:bg-white/10">
...
//         <div className="fixed bottom-6 right-6 left-6 md:left-[260px] z-20 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-white/10 bg-[#0E1422]/95 p-4 shadow-2xl backdrop-blur-xl">
//           <p className="hidden text-xs text-slate-400 md:block">
//             {activeTab === 'essl' && 'Saves API URL, Username, and Password. Biometric devices save directly inside the table.'}
//             {activeTab === 'email' && 'Saves SMTP host, port, and sender email.'}
//             {activeTab === 'rules' && 'Saves attendance thresholds and data tracking period.'}
//             {activeTab === 'automation' && 'Saves daily reporting schedule.'}
//             {activeTab === 'monthly' && 'Saves monthly reporting variables.'}
//           </p>
//           <div className="flex w-full flex-wrap justify-end gap-3 md:w-auto">
//             {activeTab === 'essl' && (
//               <Button loading={action === 'test_essl'} onClick={testEssl} type="button">
//                 Test eSSL Connection
//               </Button>
//             )}
//             {activeTab === 'email' && (
//               <Button loading={action === 'test_email'} onClick={testEmail} type="button">
//                 Send Test Email
//               </Button>
//             )}
//             <Button variant="primary" loading={action === 'save'} onClick={saveSettings} type="button">
...
//               <div>
//                 <label className="mb-1 block text-sm font-medium text-slate-300">Device Name</label>
//                 <input type="text" required value={deviceForm.name} onChange={(e) => setDeviceForm({ ...deviceForm, name: e.target.value })} className="min-h-11 w-full rounded-xl border border-white/10 bg-[#070A12]/80 px-3 text-sm text-slate-100 outline-none" />
//               </div>
//               <div>
//                 <label className="mb-1 block text-sm font-medium text-slate-300">Device Serial Number</label>
//                 <input type="text" required value={deviceForm.serial_number} onChange={(e) => setDeviceForm({ ...deviceForm, serial_number: e.target.value })} className="min-h-11 w-full rounded-xl border border-white/10 bg-[#070A12]/80 px-3 text-sm text-slate-100 outline-none" />
//               </div>
//               <div>
//                 <label className="mb-1 block text-sm font-medium text-slate-300">Device Role</label>
//                 <select value={deviceForm.role} onChange={(e) => setDeviceForm({ ...deviceForm, role: e.target.value as any })} className="min-h-11 w-full rounded-xl border border-white/10 bg-[#070A12]/80 px-3 text-sm text-slate-100 outline-none">
//                   {getAvailableRoles().map((r) => <option key={r.value} value={r.value}>{r.label}</option>)}
//                 </select>
//               </div>
//               <label className="flex items-center gap-2 pt-1">
//                 <input type="checkbox" checked={deviceForm.is_active} onChange={(e) => setDeviceForm({ ...deviceForm, is_active: e.target.checked })} className="h-4 w-4 rounded border-white/20 bg-slate-950 text-blue-500" />
//                 <span className="text-sm text-slate-300">Device Active</span>
//               </label>
//               <div className="mt-6 flex justify-end gap-3 pt-2">
//                 <button type="button" onClick={() => setDeviceModalOpen(false)} className="rounded-xl px-4 py-2 text-sm font-semibold text-slate-300 hover:bg-slate-800">Cancel</button>
//                 <Button type="submit" variant="primary" loading={savingDevice}>Save Device</Button>
//               </div>
//             </form>
//           </div>
//         </div>
...
//       <span className="text-sm font-medium text-slate-300">Report Receiver Email</span>
//       <div className="min-h-11 w-full rounded-xl border border-white/10 bg-slate-950/45 px-3 py-2 text-sm text-slate-100">
//         <span className={configured ? 'break-all font-semibold text-white' : 'text-slate-500'}>{configured ? value : 'Not configured'}</span>
//       </div>
//       <span className="text-xs leading-5 text-slate-500">This email is controlled by .env CTO_EMAIL.</span>
//     </div>
//   );
...
//         <span className="block text-xs text-slate-500">{helper}</span>
//       </span>
//       <input checked={checked} className="h-5 w-5 rounded border-white/20 bg-slate-950 text-blue-500" type="checkbox" onChange={(e) => onChange(e.target.checked)} />
//     </label>
//   );
// }
...
import { type ChangeEvent, useEffect, useState } from 'react';
import { CalendarRange, Download, FileSpreadsheet, Loader2, Mail, Save, Send, ServerCog, ShieldCheck, Timer, Plus, Trash2, Wifi, CheckCircle2, XCircle, Edit3, Home, type LucideIcon } from 'lucide-react';
...
type LoadingAction = 'save' | 'test_essl' | 'test_email' | 'generate_monthly' | 'send_monthly' | null;
type SettingsTab = 'essl' | 'email' | 'rules' | 'automation' | 'monthly' | 'self_attendance';
...
export default function AttendanceSettingsPage() {
  ...
  // === NEW: Toggle allowed type in self-attendance list ===
  function toggleAllowedType(type: string) {
    if (!settings) return;
    const currentTypes: string[] = settings.self_attendance_allowed_types || [];
    const updated = currentTypes.includes(type)
      ? currentTypes.filter((t) => t !== type)
      : [...currentTypes, type];
    setSettings({ ...settings, self_attendance_allowed_types: updated });
  }
  ...
  const tabs: { id: SettingsTab; label: string; icon: LucideIcon }[] = [
    ...
    { id: 'automation', label: 'Automation', icon: Timer },
    { id: 'monthly', label: 'Monthly Report', icon: CalendarRange },
    { id: 'self_attendance', label: 'Self-Attendance', icon: Home },  // === NEW ===
  ];
    ...
    <>
      ...
      <form className="relative pb-24" onSubmit={(e) => e.preventDefault()}>
        <div className="grid gap-5 xl:grid-cols-2">
          ...
          {/* ================= TAB 6: SELF-ATTENDANCE (NEW) ================= */}
                    {/* ================= TAB 6: SELF-ATTENDANCE ================= */}
          {activeTab === 'self_attendance' && (
            <SettingsSection title="Employee Self-Attendance">
              <SectionLabel icon={Home} text="Allow employees to add their own attendance" />

              <div className="mb-4 rounded-lg border border-blue-400/30 bg-blue-500/10 p-3 text-xs leading-5 text-blue-100">
                <strong className="mb-1 block text-blue-200">How it works:</strong>
                This is the <strong>master switch</strong>. Use the checkboxes below to control whether WFH employees, Regular employees (e.g. during device issues), or both are allowed to use this feature.
              </div>

              {/* Master Toggle */}
              <ToggleField
                label="Enable Self-Attendance"
                helper="Master switch — must be ON for any employee to self-report."
                checked={settings.enable_employee_self_attendance || false}
                onChange={(v) => updateField('enable_employee_self_attendance', v)}
              />

              {/* Target Employees */}
              {settings.enable_employee_self_attendance && (
                <>
                  <div className="mt-5">
                    <h4 className="mb-3 text-sm font-semibold text-slate-200">
                      Target Employees (Who can self-report?)
                    </h4>

                    <div className="space-y-2">
                      {/* WFH Option */}
                      <label className="flex cursor-pointer items-center justify-between gap-3 rounded-xl border border-white/10 bg-slate-950/45 p-4 hover:bg-white/[0.03]">
                        <div className="flex items-center gap-3">
                          <span className="text-2xl">🏠</span>
                          <div>
                            <p className="text-sm font-bold text-white">WFH & Hybrid Employees</p>
                            <p className="text-xs text-slate-400">
                              Allow employees with WFH/Hybrid mode to add attendance
                            </p>
                          </div>
                        </div>
                        <input
                          type="checkbox"
                          checked={(settings.self_attendance_allowed_types || []).includes('WFH')}
                          onChange={() => toggleAllowedType('WFH')}
                          className="h-5 w-5 rounded border-white/20 bg-slate-950 text-blue-500 focus:ring-blue-500"
                        />
                      </label>
                      ...
                      <label className="flex cursor-pointer items-center justify-between gap-3 rounded-xl border border-white/10 bg-slate-950/45 p-4 hover:bg-white/[0.03]">
                        <div className="flex items-center gap-3">
                          <span className="text-2xl">🏢</span>
                          <div>
                            <p className="text-sm font-bold text-white">Regular (On-Site) Employees</p>
                            <p className="text-xs text-slate-400">
                              Allow if biometric device is down or for field visits
                            </p>
                          </div>
                        </div>
                        <input
                          type="checkbox"
                          checked={(settings.self_attendance_allowed_types || []).includes('REGULAR')}
                          onChange={() => toggleAllowedType('REGULAR')}
                          className="h-5 w-5 rounded border-white/20 bg-slate-950 text-blue-500 focus:ring-blue-500"
                        />
                      </label>
                    </div>

                    {(settings.self_attendance_allowed_types || []).length === 0 && (
                      <div className="mt-3 rounded-lg border border-amber-400/30 bg-amber-500/10 p-3 text-xs text-amber-200">
                        ⚠️ No groups selected. Employees won't be able to submit self-attendance.
                      </div>
                    )}
                  </div>
                  ...
                  <div className="mt-5 rounded-lg border border-white/10 bg-slate-950/45 p-4">
                    ...
                    <ul className="space-y-1.5 text-xs text-slate-400">
                      <li>✅ Employee profile must have <strong>Allow self-attendance</strong> checked</li>
                      <li>✅ Cannot backdate more than <strong>7 days</strong></li>
                      <li>✅ Cannot submit future dates</li>
                      <li>✅ Minimum <strong>4 hours</strong> required per entry</li>
                      <li>✅ Cannot overwrite real biometric device punches</li>
                      <li>✅ All entries are labeled <code className="rounded bg-slate-800 px-1 text-slate-300">[Self-Reported]</code> in reports</li>
                    </ul>
                  </div>
                </>
              )}
        ...
        <div className="fixed bottom-6 right-6 left-6 md:left-[260px] z-20 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-white/10 bg-[#0E1422]/95 p-4 shadow-2xl backdrop-blur-xl">
          <p className="hidden text-xs text-slate-400 md:block">
            ...
            {activeTab === 'automation' && 'Saves daily reporting schedule.'}
            {activeTab === 'monthly' && 'Saves monthly reporting variables.'}
            {activeTab === 'self_attendance' && 'Saves self-attendance master toggle and allowed types.'}
          </p>
          <div className="flex w-full flex-wrap justify-end gap-3 md:w-auto">
            {activeTab === 'essl' && (
              <Button loading={action === 'test_essl'} onClick={testEssl} type="button">
