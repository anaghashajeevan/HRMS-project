...
//         {!isBlocked && day.status !== 'weekend' && day.status !== 'holiday' && <span className={`h-2 w-2 rounded-full ${style.dot}`} />}
//       </div>

//       {day.leave_info && !isBlocked && (
//         <div className="mt-1">
//           <span className="inline-block rounded px-1 py-0.5 text-[9px] font-bold text-white" style={{ backgroundColor: day.leave_info.leave_type_color }}>
//             {day.leave_info.leave_type_code} {day.leave_info.is_half_day && `(${day.leave_info.half_day_period})`}
//           </span>
//         </div>
//       )}

//       {/* ===== ALWAYS VISIBLE ON CELL: Punch In - Out & Net Hours ===== */}
//       {!isBlocked && day.status !== 'on_leave' && day.status !== 'will_be_on_leave' && day.status !== 'absent' && (
//         <div className="mt-1 flex flex-col leading-tight">
//           {(day.punch_in || day.punch_out) && (
//             <span className={`text-[9px] ${day.status === 'missing_punch' ? 'text-amber-700 font-semibold' : 'text-gray-600'}`}>
//               {day.punch_in || '--:--'} - {day.punch_out || '--:--'}
//             </span>
//           )}
//           {day.worked_hours && day.worked_hours !== '00:00' && (
//             <span className="text-[10px] font-bold text-gray-900 mt-0.5">{day.worked_hours}</span>
//           )}
//         </div>
//       )}

//       {(day.status === 'before_joining' || day.status === 'before_system_start') && (
//         <div className="mt-1 text-[8px] font-semibold text-slate-400">{day.status === 'before_joining' ? 'Not joined' : 'No data'}</div>
//       )}
...
//           <div className="mb-2 border-b border-gray-700 pb-1 text-center font-bold text-gray-100">{day.date}</div>
//           <div className="flex justify-between py-0.5">
//             <span className="text-gray-400">Status:</span>
//             <span className="font-semibold text-gray-100">{style.label}</span>
//           </div>
//           <div className="flex justify-between py-0.5">
...
import {
  ChevronLeft, ChevronRight, Loader2, Clock, TrendingDown,
  TrendingUp, Calendar as CalIcon, CheckCircle2,
} from 'lucide-react';
import toast from 'react-hot-toast';
import Sidebar from '../../components/Sidebar';
import Topbar from '../../components/Topbar';
...
export default function MyAttendancePage() {
  ...
  useEffect(() => {
    personalAttendanceApi.getSelfAttendanceConfig()
      .then(setSelfConfig)
      .catch(() => setSelfConfig({ enabled: false, work_mode: 'REGULAR', allowed_types: [] }));
  }, []);
  ...
  const handleSelfSubmit = async (e: React.FormEvent) => {
    ...
    setSavingSelf(true);
    try {
      const res = await personalAttendanceApi.addSelfAttendance({
        date: selectedDate,
        ...selfForm,
      });
      ...
      setSelfForm({
        status: selfConfig?.allowed_types[0] || 'WFH',
        punch_in: '09:30',
        punch_out: '18:30',
        reason: '',
      });
    ...
    <div className="flex h-screen bg-gray-50">
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto p-6">
          ...
          <div className="mb-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="mt-0.5 text-sm text-gray-500">
                  Track your work hours, attendance, and shortage
                  {selfConfig?.work_mode && selfConfig.work_mode !== 'REGULAR' && (
                    <span className="ml-2 inline-flex items-center rounded-full bg-indigo-100 px-2 py-0.5 text-xs font-semibold text-indigo-700">
                      {selfConfig.work_mode === 'WFH' ? '🏠 WFH Mode' : '🔄 Hybrid Mode'}
                    </span>
                  )}
                </p>
              </div>
          ...
          {/* Month Navigation */}
          <div className="mb-4 flex flex-wrap items-center justify-between gap-3 rounded-xl bg-white p-3 shadow-sm ring-1 ring-gray-100">
            <button type="button" onClick={() => navigateMonth(-1)} disabled={loading}
              className="rounded-lg p-2 text-gray-600 hover:bg-gray-100 disabled:opacity-40">
              <ChevronLeft className="h-5 w-5" />
            </button>
            <div className="flex flex-wrap items-center justify-center gap-2">
              <select value={month} disabled={loading}
                onChange={(e) => setMonth(parseInt(e.target.value, 10))}
                className="min-h-10 rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm font-semibold text-gray-900 outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 disabled:opacity-50">
                {MONTHS.map((name, index) => (
              ...
              <select value={year} disabled={loading}
                onChange={(e) => setYear(parseInt(e.target.value, 10))}
                className="min-h-10 rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm font-semibold text-gray-900 outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 disabled:opacity-50">
                {Array.from({ length: 11 }, (_, i) => today.getFullYear() - 5 + i).map((y) => (
              ...
              {data?.month_label && (
                <span className="hidden text-xs text-gray-400 sm:inline">{data.month_label}</span>
              )}
            </div>
            <button type="button" onClick={() => navigateMonth(1)} disabled={loading}
              className="rounded-lg p-2 text-gray-600 hover:bg-gray-100 disabled:opacity-40">
          ...
          {loading || !data ? (
            ...
            <>
              {/* Stats Cards */}
              <div className="mb-4 grid grid-cols-2 gap-3 md:grid-cols-4">
                <StatCard label="Total Worked" value={data.stats.total_worked_hours}
                  helper={`${data.stats.total_worked_hours_decimal}h total`} icon={Clock} color="blue" />
                <StatCard label="Expected" value={`${data.stats.expected_hours}h`}
                  helper={data.stats.on_leave_days > 0
                    ? `${data.stats.effective_working_days} days × ${data.stats.full_day_hours}h (excl. ${data.stats.on_leave_days} leave)`
                    : `${data.stats.working_days_elapsed} days × ${data.stats.full_day_hours}h`}
                  icon={TrendingUp} color="green" />
                <StatCard label="Shortage"
                  value={data.stats.shortage_hours > 0 ? `${data.stats.shortage_hours}h` : '✅ None'}
                  helper={data.stats.shortage_hours > 0 ? 'Hours below expected' : 'On track!'}
                  icon={TrendingDown} color={data.stats.shortage_hours > 0 ? 'red' : 'green'} />
                <StatCard label="Attendance %" value={`${data.stats.attendance_percent}%`}
                  helper={`${data.stats.present_days} of ${data.stats.working_days_elapsed} days`}
                  icon={CheckCircle2} color={data.stats.attendance_percent >= 90 ? 'green' : 'amber'} />
              </div>
              <div className="mb-4 grid grid-cols-2 gap-3 md:grid-cols-5">
                <MiniStat label="Present" value={data.stats.present_days} color="green" />
                <MiniStat label="Absent" value={data.stats.absent_days} color="red" />
              ...
              {data.stats.expected_hours > 0 && (
                <div className="mb-4 rounded-xl bg-white p-5 shadow-sm ring-1 ring-gray-100">
                  <div className="mb-2 flex items-center justify-between">
                    <p className="text-sm font-semibold text-gray-900">Monthly Progress</p>
                    <p className="text-xs text-gray-500">
                      {data.stats.total_worked_hours_decimal}h / {data.stats.expected_hours}h
                    </p>
                  </div>
                  <div className="h-3 overflow-hidden rounded-full bg-gray-100">
                    <div className={`h-full transition-all ${
                      data.stats.total_worked_hours_decimal >= data.stats.expected_hours ? 'bg-green-500'
                        : data.stats.total_worked_hours_decimal / data.stats.expected_hours >= 0.9 ? 'bg-blue-500'
                        : data.stats.total_worked_hours_decimal / data.stats.expected_hours >= 0.7 ? 'bg-amber-500'
                        : 'bg-red-500'
                    }`} style={{
                      width: `${Math.min(100, (data.stats.total_worked_hours_decimal / data.stats.expected_hours) * 100)}%`,
                    }} />
                  </div>
                  {data.stats.shortage_hours > 0 && (
                    <p className="mt-2 text-xs text-red-600">
                ...
                <div className="grid grid-cols-7 gap-2">
                  ...
                  {data.days.map((day) => (
                    <DayCard
                      key={day.date}
                      day={day}
                      canAddSelf={selfConfig?.enabled || false}
                      onClick={() => {
                        if (day.status === 'before_joining' || day.status === 'before_system_start') return;
                        if (day.is_future || day.status === 'weekend' || day.status === 'holiday') return;

                        // Self-attendance modal for absent/missing days
                        if (
                          selfConfig?.enabled &&
          ...
          {selfConfig?.enabled && selfModalOpen && (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
              <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
                <div className="mb-1 flex items-center gap-2">
                  ...
                  <h3 className="text-lg font-bold text-gray-900">Add My Attendance</h3>
                </div>
                <p className="mb-4 text-sm text-gray-500">
                  Report your work attendance for <strong>{selectedDate}</strong>
                </p>

                <form onSubmit={handleSelfSubmit} className="space-y-4">
                  <div>
                    <label className="mb-1 block text-sm font-medium text-gray-700">Attendance Type</label>
                    <select value={selfForm.status}
                      onChange={(e) => setSelfForm({ ...selfForm, status: e.target.value })}
                      ...
                      {selfConfig.allowed_types.map((type) => (
                        <option key={type} value={type}>
                          {type === 'WFH' ? '🏠 Work From Home' : type === 'Site Visit' ? '🏗️ Site Visit' : type}
                        </option>
                      ))}
                    </select>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input type="time" required value={selfForm.punch_in}
                        onChange={(e) => setSelfForm({ ...selfForm, punch_in: e.target.value })}
                        className="w-full rounded-lg border border-gray-300 p-2.5 text-sm outline-none focus:border-blue-500" />
                    </div>
                    <div>
                      <label className="mb-1 block text-sm font-medium text-gray-700">End Time</label>
                      <input type="time" required value={selfForm.punch_out}
                        onChange={(e) => setSelfForm({ ...selfForm, punch_out: e.target.value })}
                        className="w-full rounded-lg border border-gray-300 p-2.5 text-sm outline-none focus:border-blue-500" />
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <textarea required minLength={5}
                      placeholder="e.g. WFH approved by Manager — working on client deliverable"
                      value={selfForm.reason}
                      onChange={(e) => setSelfForm({ ...selfForm, reason: e.target.value })}
                      className="w-full rounded-lg border border-gray-300 p-2.5 text-sm outline-none focus:border-blue-500"
                      rows={3} />
                    <p className="mt-1 text-xs text-gray-400">Visible to HR and your manager</p>
                  </div>

                  <div className="mt-6 flex justify-end gap-3 border-t border-gray-100 pt-4">
                    <button type="button" onClick={() => setSelfModalOpen(false)}
                      className="rounded-lg px-4 py-2 text-sm font-semibold text-gray-600 hover:bg-gray-100">
                      Cancel
                    </button>
                    <button type="submit" disabled={savingSelf}
                      ...
                      {savingSelf && <Loader2 className="h-4 w-4 animate-spin" />}
                      Submit Attendance
                    </button>
                  </div>
                </form>
...
function StatCard({ label, value, helper, icon: Icon, color }: {
  label: string; value: string; helper: string; icon: any;
  color: 'blue' | 'green' | 'red' | 'amber';
}) {
  const colorMap = {
...
function DayCard({ day, onClick, canAddSelf = false }: {
  day: DayEntry; onClick?: () => void; canAddSelf?: boolean;
}) {
  const style = STATUS_STYLES[day.status] || STATUS_STYLES.future;
  const isBlocked = day.status === 'before_joining' || day.status === 'before_system_start'
    || day.is_future || day.status === 'weekend' || day.status === 'holiday';
  const canSelfAdd = canAddSelf && !isBlocked
    && (day.status === 'absent' || day.status === 'missing_punch');
  const clickable = !isBlocked && Boolean(onClick);
  ...
  return (
    <div onClick={clickable ? onClick : undefined}
      ...
      {day.leave_info && !isBlocked && (
        <div className="mt-1">
          <span className="inline-block rounded px-1 py-0.5 text-[9px] font-bold text-white"
      ...
      {canSelfAdd && (
        <span className="mt-1 block text-[9px] font-bold text-indigo-600">+ Add</span>
      )}
      ...
      {!isBlocked && (
        <div className="pointer-events-none absolute bottom-full left-1/2 z-50 mb-2 hidden w-44 -translate-x-1/2 flex-col rounded-lg bg-gray-900 p-3 text-xs text-white shadow-xl opacity-0 transition-opacity group-hover:flex group-hover:opacity-100">
          ...
          <div className="absolute -bottom-1 left-1/2 h-3 w-3 -translate-x-1/2 rotate-45 bg-gray-900" />
        </div>
      )}
    </div>
  );
