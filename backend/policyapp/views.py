...
# from .utils import stamp_pdf_document
...
#             if content_file and content_type == 'HTML':
#                 ext = content_file.name.lower().split('.')[-1]
#                 if ext == 'pdf':
#                     content_type = 'PDF'
...
#     def _get_approval_data(self, policy):
#         version = policy.current_version
#         from .models import PolicyApproval
...
#         steps = []
#         if policy.created_by and hasattr(policy.created_by, 'user'):
#             u = policy.created_by.user
#             full_name = f"{u.first_name} {u.last_name}".strip() or u.username
#             steps.append({
#                 'role': 'PREPARED BY',
#                 'name': full_name,
#                 'date': version.created_at.strftime('%Y-%m-%d') if version else timezone.now().strftime('%Y-%m-%d'),
#                 'emp_id': getattr(policy.created_by, 'employee_code', '') or getattr(policy.created_by, 'employee_id', '') or ''
#             })

#         for app in approvals:
#             if hasattr(app.approver, 'user'):
#                 u = app.approver.user
#                 full_name = f"{u.first_name} {u.last_name}".strip() or u.username
#             else:
#                 full_name = "Approver"
...
#                 'role': 'APPROVED BY',
#                 'name': full_name,
#                 'date': app.approved_at.strftime('%Y-%m-%d') if app.approved_at else 'N/A',
#                 'emp_id': getattr(app.approver, 'employee_code', '') or getattr(app.approver, 'employee_id', '') or ''
#             })

#         if policy.published_by and hasattr(policy.published_by, 'user'):
#             u = policy.published_by.user
#             full_name = f"{u.first_name} {u.last_name}".strip() or u.username
#             steps.append({
#                 'role': 'PUBLISHED BY',
#                 'name': full_name,
#                 'date': policy.published_at.strftime('%Y-%m-%d') if policy.published_at else 'N/A',
#                 'emp_id': getattr(policy.published_by, 'employee_code', '') or getattr(policy.published_by, 'employee_id', '') or ''
#             })
...
#             'steps': steps,
#             'author_name': f"{policy.created_by.user.first_name} {policy.created_by.user.last_name}".strip() if (policy.created_by and hasattr(policy.created_by, 'user')) else 'Admin',
#             'published_at': policy.published_at.strftime('%Y-%m-%d') if policy.published_at else timezone.now().strftime('%Y-%m-%d')
#         }
...
#             return Response({'detail': 'No file available for download'}, status=status.HTTP_404_NOT_FOUND)

#         file_path = version.file.path
#         file_name = f"{policy.title.replace(' ', '_')}_v{version.version_number}"

#         if file_path.lower().endswith('.pdf'):
#             approval_data = self._get_approval_data(policy)
#             pdf_io = stamp_pdf_document(file_path, approval_data)
#             response = HttpResponse(pdf_io.read(), content_type='application/pdf')
#             response['Content-Disposition'] = f'attachment; filename="{file_name}.pdf"'
#             return response

#         ext = file_path.split('.')[-1]
#         return FileResponse(open(file_path, 'rb'), as_attachment=True, filename=f"{file_name}.{ext}")
...
#         if not version or not version.file:
#             return Response({'detail': 'No file available for preview'}, status=status.HTTP_404_NOT_FOUND)
...
#                 effective_from=effective_from, content_file=content_file,content_type=content_type,
#             )
...
from .services.policy_service import PolicyService, PolicyServiceError
from .utils import stamp_pdf_document, get_or_create_pdf_version, ConversionError
...
class PolicyCategoryViewSet(ModelViewSet):
    ...
    def get_permissions(self):
        ...
        if self.action in [
            'approve', 'reject', 'return_for_changes',
            'pending_approvals'
        ]:
            return [IsAuthenticated()]
...
class PolicyViewSet(ModelViewSet):
    ...
    def get_permissions(self):
        if self.action == 'preview':
            return []  # Allow browser iframe request to open directly
        ...
        if self.action in ['approve', 'reject', 'return_for_changes', 'pending_approvals']:
            return [IsAuthenticated()]
    ...
    def get_queryset(self):
        ...
        # Anonymous requests (e.g. iframe preview) — no role/employee to check
        if not user.is_authenticated:
            if self.action == 'preview':
                # Preview is intentionally public (permission_classes=[]);
                # restrict to published+active policies for safety
                return qs.filter(status='PUBLISHED', is_active=True)
            return qs.none()
    ...
    # ---------- DOCUMENT PREVIEW & DOWNLOAD WITH STAMPS ----------

    def _resolve_full_name(self,user_account, employee=None):
        """Best-effort name resolution that won't crash on schema mismatches."""
        if user_account:
            # Try common name field patterns
            full_name = getattr(user_account, 'full_name', None)
            if full_name:
                return full_name
            first = getattr(user_account, 'first_name', '') or ''
            last = getattr(user_account, 'last_name', '') or ''
            if (first or last):
                return f"{first} {last}".strip()
            for candidate in ('username', 'email', 'name'):
                val = getattr(user_account, candidate, None)
                if val:
                    return val
        # Fall back to the employee record itself
        if employee:
            emp_name = getattr(employee, 'full_name', None) or getattr(employee, 'name', None)
            if emp_name:
                return emp_name
        return 'Unknown'
    ...
    def _get_approval_data(self, policy):
        ...
        approvals = PolicyApproval.objects.filter(
            version=version, status='APPROVED'
        ).select_related('approver__user_account')

        steps = []
        if policy.created_by:
            u = getattr(policy.created_by, 'user_account', None)
            steps.append({
                'role': 'PREPARED BY',
                'name': self._resolve_full_name(u, policy.created_by),
                'date': version.created_at.strftime('%Y-%m-%d') if version else timezone.now().strftime('%Y-%m-%d'),
                'emp_id': getattr(policy.created_by, 'employee_code', '') or getattr(policy.created_by, 'employee_id', '') or ''
            })

        for app in approvals:
            u = getattr(app.approver, 'user_account', None)
            steps.append({
                'role': 'APPROVED BY',
                'name': self._resolve_full_name(u, app.approver),
                'date': app.acted_at.strftime('%Y-%m-%d') if app.acted_at else 'N/A',
                'emp_id': getattr(app.approver, 'employee_code', '') or getattr(app.approver, 'employee_id', '') or ''
            })
        ...
        return {
            ...
            'version': version.version_number if version else '1.0',
            'steps': steps,
            'author_name': self._resolve_full_name(
                getattr(policy.created_by, 'user_account', None) if policy.created_by else None,
                policy.created_by
            ),
            'published_at': policy.published_at.strftime('%Y-%m-%d') if policy.published_at else timezone.now().strftime('%Y-%m-%d')
        }
    ...
    def download(self, request, pk=None):
        ...
        if not version or not version.content_file:
            return Response({'detail': 'No file available for download'}, status=status.HTTP_404_NOT_FOUND)
    ...
    @action(detail=True, methods=['get'], url_path='preview', permission_classes=[])
    def preview(self, request, pk=None):
        policy = self.get_object()
        version = policy.current_version
